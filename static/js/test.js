...
$(function() {
  ...
  $('.widget1').click(() => {
    if ($('.widget-category-list').is(':visible') && logo !== 0) {
      $('.widget-category-list').slideUp(400, () => {
        $('.widget1').html('Доступные лиги ↓');
      });
    } else if (logo !== 0) {
      $('.widget-category-list').slideDown('slow', () => {
        $('.widget1').html('Доступные лиги ↑');
      });
    }
  });
  $('.widget2').click(() => {
    if ($('.widget-posts-list').is(':visible') && start !== 0) {
      $('.widget-posts-list').slideUp(400, () => {
        $('.widget2').html('Предстоящие матчи ↓');
      });
    } else if (start !== 0) {
      $('.widget-posts-list').slideDown('slow', () => {
        $('.widget2').html('Предстоящие матчи ↑');
      });
    }
  });
...
function timerStart(resp) {
  ...
  if (timer == 0)
    timer = setTimeout(secondAntillBlock * 1000 + 1, () => {
      timer = 0;
    });
}
...
function addComp() {
  Competitions.forEach(item => {
    ...
    newLi.innerHTML = `<input type="checkbox" name="competition" value="${
      item.id
...
function getMatches(stepDay = 1) {
  ...
  if (dateFrom.getMonth() + 1 < 10) monthFrom = '0' + (dateFrom.getMonth() + 1);
  else monthFrom = dateFrom.getMonth() + 1 + '';
  if (dateFrom.getDate() < 10) dayFrom = '0' + dateFrom.getDate();
  else dayFrom = dateFrom.getDate() + '';
  let dateTo = dateFrom;
  dateTo.setDate(dateFrom.getDate() + stepDay);
  if (dateTo.getMonth() + 1 < 10) monthTo = '0' + (dateTo.getMonth() + 1);
  else monthTo = dateTo.getMonth() + 1 + '';
  if (dateTo.getDate() < 10) dayTo = '0' + dateTo.getDate();
  else dayTo = dateTo.getDate() + '';
  $('.loadlogo').show();
  httpGet(
    `http://api.football-data.org/v2/competitions/2021/matches?dateFrom=2018-${monthFrom}-${dayFrom}&dateTo=2018-${monthTo}-${dayTo}`
  )
    .then(
      resp => {
        secondAntillBlock = resp.getResponseHeader('x-requestcounter-reset');
...
function addMatch() {
  Matches.matches.forEach((item, i) => {
    ...
    newLi.innerHTML = `<div class="post-image-small">
          <strong>${+item.utcDate.slice(11, 13) +
            3 +
            item.utcDate.slice(13, 16)} ${item.utcDate.slice(8, 10) +
      '.' +
      item.utcDate.slice(5, 7)}</strong>
        </div>
...
function startMongo() {
  mongoGet(
    'mongodb://amalyshok:S1pa2Wn3@footballdata-zxox4.mongodb.net/test?retryWrites=true'
